...
| Create a Deployment for a Web application of your choice, that uses the REST API. It is expected that you create your own Dockerfile for this image. Ensure that the application can scale horizontally. Create an appropriate type of Service/Ingress/API Gateway, such that the application can be accessed by users outside of the cluster.| NIL |
| Configure TLS for the web application. You can use self-signed certificates or certificates signed by a self-made certificate authority. You can use cert-manager and/or openssl to generate the certificates. Ensure that if the REST API is accessible outside of the Cluster, it is running on https as well. Exposing http endpoints that redirect to https is fine.| NIL |
| Configure ServiceMesh (Istio). If you use the Istio Ingress Gateway, it's ok not to configure Kubernetes NGINX Ingress as it replaces it.| NIL |
| Configure application monitoring with Prometheus. In the presentation, you need to show how you query Prometheus. | [Install](#installing-prometheus) [Query](#query-prometheus) |
| Run the Application on Google Cloud Platform. | [GKE](#creating-the-gke-cluster) |
